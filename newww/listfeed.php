...
$s=mysqli_query($con,"select * from tbl_feedback");
			echo"<table border ='1' cellspacing=3 width=80% height=80% align='center' style=margin:90px; bgcolor=#f7aa77>
<tr>
<th>Email</th><th>Rate our web?</th><th>Rate design of web?</th><th>Service?</th><th>Information provided enough?</th><th>User friendly?</th><th>Suggestion?</th><th>Remove</th>
</tr>";
 while($row=mysqli_fetch_array($s))
  {
	  $fid=$row[0];
  echo"<tr> <a href='listfeed.php'></td>
  <td width='7%'>$row[2]</td><td width='11%'>$row[4]</td><td width='11%'>$row[5]</td><td width='11%'>$row[6]</td><td width='11%'>$row[7]</td><td width='11%'>$row[8]</td><td width='11%'>$row[9]</td>
  <td width='7%'><form method='post' action='feedbackdelete.php'><input type='hidden' name='h1' value='$fid'> <a href='feedbackdelete.php'><input type='image' name='imgbtn' src='images/aaaaaaaaaaaa.jpg' width=10 hight=10 value=delete></a>
    </form>
  </tr>";
  }
